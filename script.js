...
(function(){
  ...
  function isRtl(lang){
    ...
    return rtlLangs.has(code);
  }

  function applyLanguageDirection(lang){
    const rtl = isRtl(lang);
    // Ensure html has the language set
  ...
  const observer = new MutationObserver((mutations) => {
    for(const m of mutations){
      if(m.type === 'attributes' && m.attributeName === 'lang'){
        applyLanguageDirection(document.documentElement.lang || 'en');
      }
    }
  });
  ...
  // Expose helper for programmatic changes
  window.setPageLanguage = function(lang){
    if(!lang) return;
    document.documentElement.lang = lang;
    // applyLanguageDirection will be triggered via MutationObserver, but call directly to be immediate
    applyLanguageDirection(lang);
    // Dispatch a custom event for consumers
    document.dispatchEvent(new CustomEvent('page-language-changed', { detail: { lang } }));
  };
})();
